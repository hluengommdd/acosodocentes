/* Generic print renderer
   - Call `renderPrint(data)` with a `data` object to render custom content using
     the existing print styles.
   - If no `data` is passed, it will attempt to read `printData` from sessionStorage
     and fallback to localStorage.
   - Designed so another platform can include this file and only supply a different
     `printData` payload (or call `renderPrint`) without changing CSS.
*/
(function(window){
  function readStored(){
  ...
  async function setupControls(){
    if(document.getElementById('printControls')) return;
    ...
    document.getElementById('doPdf').addEventListener('click', async ()=>{
      ...
      try{ await html2pdf().set(opt).from(document.getElementById('printContent')).save() }catch(err){ console.error(err); window.print(); }
    })
    document.getElementById('doPrint').addEventListener('click', ()=>window.print())
  }

  async function renderPrint(data){
    const d = data || readStored();
  ...
  if(document.readyState === 'complete' || document.readyState === 'interactive'){
    // slight delay to allow other scripts to set storage
    setTimeout(()=>{ if(window.renderPrint) window.renderPrint(); }, 50);
  }
